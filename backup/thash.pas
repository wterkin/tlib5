...
const
      {$region ' CRC'}
      cadwTable: array[0..255] of DWORD =
...
{** @param(ppBuffer - буфер, CRC которого нужно посчитать)
    @param(pdwByteCount - размер буфера)
    @param(pdwCRCValue - возвращаемое значение)
    @link(CalcStringCRC32)
    @link(CalcFileCRC32)  }
procedure CalcCRC32(ppBuffer : Pointer; pdwByteCount : DWORD; var pdwCRCValue : DWORD);


{** @abstract Функция подсчитывает контрольную сумму строки.
    @param(psLine - строка, CRC которой нужно вычислить)
    @param(pdwCRC32 - возвращаемое значение контрольной суммы)
    @returns(а штоб я знал, что оно возвращает :] )
    @link(CalcCRC32)
    @link(CalcFileCRC32)  }
...
{** @abstract Функция подсчитывает контрольную сумму файла.
    @param(psFromName - имя файла, CRC которого нужно вычислить)
    @param(pdwCRCvalue - возвращаемое значение контрольной суммы)
    @returns(@true в случае успешного выполнения, иначе @false)
    @link(CalcCRC32)
    @link(CalcStringCRC32) }
...
begin
  ...
  if ldwCRC32Table=$6FCF9E13 then
  begin
...
begin
  ...
  try
    ...
    if loStream.Size>0 then
    begin

      CalcCRC32(loStream.Memory,loStream.Size,pdwCRCvalue);
